...
SELECT
	...
	(
	SELECT
		count (newcomer) AS number_of_new_comers
	FROM
		global_500
	WHERE
		newcomer = 'yes'
		AND YEAR = g.year) AS count_newcomers,
...
SELECT
	year,
	CAST(((count_newcomers * 1.0 / total)* 100) AS DECIMAL(10,
	2)) AS perc_newcomers
FROM
	prep_newcomers
GROUP BY
...
SELECT
	...
	(
	SELECT
		Count (ceowoman) AS number_of_ceo_woman
	FROM
		global_500
	WHERE
		ceowoman = 'yes'
		AND year = g.year) AS count_ceowoman,
...
SELECT
	year,
	CAST(((count_ceowoman * 1.0 / total)* 100) AS DECIMAL(10,
	2)) AS perc_ceowoman
FROM
	prep_ceo_woman
GROUP BY
...
SET revchange = ( SELECT 
CASE
    WHEN ROUND((((revenues - prev_revenues)/prev_revenues) * 100),5) IS NOT null THEN ROUND((((revenues - prev_revenues)/prev_revenues) * 100),5)
...
SELECT  name, YEAR, revenues,
revchange, profits, lAG(profits ,1) OVER (PARTITION BY name ORDER BY YEAR ) AS prev_profits, prftchange
FROM global_500
GROUP BY name , YEAR
...
SELECT
	* ,
	MIN((rating - prev_rank)) AS rating_changes
FROM
	prep_table
